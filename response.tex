...
\noindent\textbf{Response.} Corrected \nowat{viva:c2-section223}. The revised sentence now reads: ``As discussed in the Hypothesis Development section (Section~2.2.3), receiving adverse internal control opinions\ldots''.
...
\noindent\textbf{Response.} Corrected \nowat{viva:c2-icosdef}. The revised definition in the Appendix now reads: ``(1)~the predicted probabilities of receiving MW when dismissal is set to 1 is \emph{less than} the predicted probabilities when dismissal is set to 0 and dismissal is actually 1; (2)~the predicted probabilities of receiving MW when dismissal is set to 0 is \emph{less than} the predicted probabilities when dismissal is set to 1 and dismissal is actually 0.'' The two conditions are now properly stated as $P_1 < P_0$ (with actual dismissal) and $P_0 < P_1$ (i.e., $P_1 > P_0$, with actual retention), consistent with the Lennox (2000) framework.
...
\noindent\textbf{Response.} Corrected \nowat{viva:c3-campello}. The sentence now reads: ``Campello and Gao (2017) \emph{examine} how the credit market assesses a firm's customer-base profile\ldots''.
